  ...
  <view class="page-detail">
    <view class="banner">
      <view class="banner_swiper"
        ><image :src="datail.banner" mode="aspectFill"></image
      ></view>
    </view>
    ...
    <view class="content">
      <view class="top bg-fff uni-pa-5">
        ...
        <view class="discount" v-show="hasPay == 1">
          <view class="discount_money">
            <view class="discount_surprice" v-if="datail.price == null"
              >￥0</view
            >
            <view class="discount_surprice" v-else
              >￥{{ toFixed(datail.price) }}</view
            >
            <view class="discount_price" v-if="datail.showPrice == null"
              >￥0</view
            >
            <view class="discount_price" v-else
              >￥{{ toFixed(datail.showPrice) }}</view
            >
          </view>
        </view>
      ...
      <view class="user_box uni-mt-5 uni-py-5">
        <view class="user_list">
          ...
          <view class="user_item">
            ...
            <view class="user_item_top" v-else-if="datail.stage == 0"
              >完结</view
            >
            <view class="user_item_top" v-else-if="datail.stage == null"
              >完结</view
            >
      ...
      <view class="course_content uni-mt-5">
        <view class="course_content_menu">
          <view
            :class="content_menu == 1 ? 'a1 active' : 'a1'"
            @click="content_menu = 1"
            >简介</view
          >
          <view
            :class="content_menu == 2 ? 'a2 active' : 'a2'"
            @click="content_menu = 2"
            >目录</view
          >
        </view>
        <view class="course_ccc" v-show="content_menu == 1">
          <view class="rich-text-box" style="padding: 40rpx"
            ><rich-text :nodes="datail.strings"></rich-text
          ></view>
        </view>
        ...
        <view class="course_list" v-show="content_menu == 2">
          ...
          <view
            class="course_list_item uni-py-5 uni-mx-5"
          ...
          >
            <view class="item_left">
              ...
              <view class="course_box">
                <view class="list_item_title"
                  ><view class="list_item_tt">{{ data.title }}</view></view
                >
                <view class="list_item_content">
                  <view class="l_t_bq" v-if="data.type == 'VIDEO'">视频</view>
                  <view class="l_t_bq" v-else-if="data.type == 'VOICE'"
                    >音频</view
                  >
                  <view class="l_t_bq" v-else-if="data.type == 'IMG'"
                    >图片</view
                  >
            ...
            <!--  这条章节免费 -->
            <view
              class="right watch trywatch"
              v-if="data.free == true && hasPay == 1"
              @click="GoStudyTry({ index: index })"
              >试看</view
            >
            <view
              class="right watch"
      ...
      <view class="notice uni-pa-5 uni-mt-5">
        ...
        <view class="notice_item">
          ...
          <view class="item_text"
            >平台搜索【闪电课堂】进入小程序，点击底部【学习】按钮，即可观看已购买课程。</view
          >
        </view>
        <view class="notice_item">
          <view class="item_title">
...
export default {
  ...
  onLoad(e) {
    ...
    console.log(uni_options.scene, "场景值")
		// 我的订单【去学习】按钮进入到此页面跳转至章节学习页面
    if (uni_options.scene === "021013") {
      console.log("订单综合入口")
      uni.redirectTo({
				url: `/pages/detail/detail?course_id=${e.course_id}&login=${1}`,
      })
      return
    }
    if (e.course_id) {
